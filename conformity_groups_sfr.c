...
int main(int argc, char **argv)
{
  ...
  float *mag_r, *mag_g, dx, dy, dz, x1, theta, phi, theta_max, rmax, *ra, *dec, *redshift, *mass, *rad, *sigma, *ang_rad, *mgal, *grcolor, *colorgrp, *sfrgrp;
  ...
  double mbar[160];
  double sfrcen[20], sfrsat[20], sfrcenall[20], sfrsatall[20], cenmass[20], satmass[20];
  double sfrcen_jack[50][50], sfrsat_jack[20][50], sfrcenall_jack[20][50], sfrsatall_jack[20][50];
  double mgrpbar, mgrpbin;
  ...
  float *psat, *rcogrp, *mgrp, *ragrp, *decgrp, *zgrp, xx[100], x2, dr, *radgrp, *dn4k, *dn4ktemp,
    *fquench,*fqsat,*fqcen,*dn4kgrp, *sfr;
  ...
  fprintf(stderr,"input: %d %f %f %d %f %f\n",imag, MLO, CENTHRESH, BLUE, DELTA_M, THRESHOLD);

  //read in all the galaxies from the VAGC
  if(imag==0)
    fp = openfile("/Users/tinker/cosmo/CENSAT_DECOMP/DR7_RESULTS/clf_groups_M20_M10.2.galdata_corr");
  if(imag==1)
  ...
  for(i=1;i<=ngal;++i) 
    {
      fscanf(fp,"%s %d %f %f %f %f %f %f %f %f %f",aa,&idum,&mag_r[i],&mag_g[i],&redshift[i],&dn4k[i],
	     &x1,&sfr[i],&mgal[i],&ra[i],&dec[i]);      
      indx[i] = i;
      mgal[i] = log10(mgal[i]);
      redshift[i] = redshift[i]/SPEED_OF_LIGHT;
      grcolor[i] = mag_g[i] - mag_r[i];
  ...
  psat = vector(1,ngal);
  if(imag==0) 
    fp = openfile("/Users/tinker/cosmo/CENSAT_DECOMP/DR7_RESULTS/clf_groups_M20_M10.2.prob");
  if(imag==1) 
  ...
  */
  ...
  dn4ktemp = vector(1,ngrp);
  colorgrp = vector(1,ngrp);
  sfrgrp = vector(1,ngrp);
  ...
  for(j=1;j<=ngal;++j)
    {
      ...
      if(mgal[j]<MLO || mgal[j]>MLO+DELTA_M)continue;
      i++;
      ...
      ijack[i] = xg[j];
      mgrpbar += pow(10,mgal[j]);
    }
  ngrp = i;
  fprintf(stderr,"numgrps: %d\n",ngrp);
  
  // sort all values of dn4k to get breakpoint
  sort(ngrp, dn4ktemp);
  x1 = THRESHOLD;
  i = (int)((THRESHOLD)*ngrp);
  THRESHOLD = dn4ktemp[i]; // higher dn4k is lower SFR
  fprintf(stderr,"sfr threshold = %f %d %.1f\n",THRESHOLD,i,ngrp*(x1));
  ...
  for(i=1;i<=nbins;++i)
    {
      cenmass[i] = satmass[i] = 0;
      sfrcen[i] = sfrsat[i] = 0;
      for(j=0;j<25;++j)
	sfrcen_jack[i][j] = sfrsat_jack[i][j] = 0;
    }
  ...
  for(j=1;j<=ngrp;++j)
    {
      ...
      // decision tree on sSFR
      if(!BLUE)
	if(sfrgrp[j]>THRESHOLD)continue;
      if(BLUE)
	if(sfrgrp[j]<THRESHOLD)continue;
	
      ncnt++;
      mgrpbin += pow(10.0,mgrp[j]);
	...
	{	  
	  ...
	  if(psat[i]>0) {
	    ...
	    satcnt_jack[ibin][ii]++;
	    sfrsat[ibin]+=sfr[i];
	    sfrsat_jack[ibin][ii] += sfr[i];
	    satmass[ibin]+= pow(10.0,mgal[i]);
	  }
	  ...
	  if(psat[i]<PLIM) {
	    ...
	    sfrcen[ibin] += sfr[i];
	    sfrcen_jack[ibin][ii] += sfr[i];
	    cenmass[ibin]+= pow(10.0,mgal[i]);
	  }

	}
    }
  fprintf(stderr,"%d clusters %f %f\n",ncnt, log10(mgrpbar/ngrp),log10(mgrpbin/ncnt));
  ...
  for(i=1;i<=nbins;++i)
	...
	for(j=0;j<25;++j)
	  {
	    x1 = sfrcen[i]/galcnt[i];
	    x2 = (sfrcen[i]-sfrcen_jack[i][j])/(galcnt[i]-galcnt_jack[i][j]);
	    err_qc += (x2-x1)*(x2-x1);

	    x1 = sfrsat[i]/satcnt[i];
	    x2 = (sfrsat[i]-sfrsat_jack[i][j])/(satcnt[i]-satcnt_jack[i][j]);
	    err_qs += (x2-x1)*(x2-x1);
	  }
	err_qc = sqrt(25./24.*err_qc);
	err_qs = sqrt(25./24.*err_qs);

	printf("%d %f %f %f %f %f %f %f\n",i,rbar[i]/(galcnt[i]+satcnt[i]),sfrcen[i]/galcnt[i],err_qc,
	       sfrsat[i]/satcnt[i],err_qs,log10(cenmass[i]/galcnt[i]),log10(satmass[i]/satcnt[i]));
      }	  
}
