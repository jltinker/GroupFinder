...
int main(int argc, char **argv)
{
  ...
  */
  ...
  for(i=1;i<=ngal;++i)
	...
	printf("INDX %d %f %f %f %f %f %f %e %f %f %f %f %f\n",
	       i,mag_r[i],mag_g[i],redshift[i],Dn4k[i],H_delta[i],
	       SFR[i],mstellar[i],ra[i],dec[i],vdisp[i],s2n[i],sersic[i]);
    }
  fflush(stdout);
  ...
  for(i=1;i<=ngal;++i)
    {
      if(!(mag_r[i]>MAGNITUDE || redshift[i]>REDSHIFT*SPEED_OF_LIGHT || redshift[i]<CZMIN))
	{
	  // if(icollided[i]){ mag_r[i]=-1; continue; } // this is to run with NO FIBER COLLIDED GALAXIES
	  ...
	  printf("GALDAT_CORR %d %f %f %f %f %f %f %e %f %f %f %f %f\n",
		 i,mag_r[i],mag_g[i],redshift[i],x1,x2,x3,
		 mstellar[i],ra[i],dec[i], vdisp[i],s2n[i], sersic[i]);
	  fflush(stdout);
	}
    }
  fflush(stdout);
  ...
  for(niter=1;niter<=niter_max;++niter)
      ...
      for(i1=1;i1<=ngrp;++i1)
	{
	  ...
	  // NB!! testing using most massive galaxy as group center
	  if(nsat_indi[i]>=2)
	    {
	      //j = central_galaxy(i,ra,dec,redshift,mag_r,ang_rad[i],sigma[i],
	      //		 group_member,indx,ngal,ang_rad[i],mass[i], igrp, luminosity,&nsat_indi[i],i1,prob_total);
	      maxlum = 0;
	      for(j=1;j<=ngal;++j)
		{
		  if(group_member[j]==igrp)
		    if(mstellar[j]>maxlum) 
		      {
			maxlum = mstellar[j];
			imax = j;
		      }
	      ...
	      if(imax!=i)
		{
		  group_center[igrp] = imax;
		  i = imax;
		}
	      printf("CEN%d %d %d %d %f %f %e %e\n",
		     niter,igrp,i,imax,luminosity[i],luminosity[imax],mstellar[i],mstellar[imax]);
	    }
