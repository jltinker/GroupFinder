...
int main(int argc, char **argv)
{
  ...
  int i, j, k,i1, j1, nmatched, idum, n1, n2, ngal, ngal_sample, nheader;
  ...
  //fp = openfile("input_catalog_shuffle_3e9.dat");
  fp = openfile("input_UM_MDPL_z0.36.dat");
  ngal = filesize(fp);
  ...
  for(i=1;i<=ngal;++i) 
    {
      ...
      //	     &ra[i],&dec[i],&redshift[i],&x1,&x1,&vz,&x1,&mag_r[i],&icen,&x1,&x1,&idum,
      //     &hostmass,&idum,&j);
      // for Andrew Hearin's mock:
      fscanf(fp,"%f %f %f %f %f %f %f",&ra[i],&dec[i],&redshift[i],&x1,&x2,&vz,&mag_r[i]);
  ...
  for(niter=1;niter<=niter_max;++niter)
      ...
      for(i=1;i<=ngal;++i)
	{
	  ...
	  printf("PROB%d %d %d %d %e %e %e %e %e %e %e %e %e %d\n",niter,i,igrp,group_center[igrp],mag_r[j1],
		 prob_total[i],mass[i],nsat_indi[i],mass[j],nsat_indi[j],theta/ang_rad[j],theta,ang_rad[j],subid[j]);
	}
  ...
  for(j1=1+id;j1<=ngal;j1+=nstep)
    {
      ...
      if(dz>REDSHIFT/2)dz = REDSHIFT-dz;
      if(dz>6*x1)continue;
...
float find_satellites3(int i, float *ra, float *dec, float *redshift, float *mag_r, float theta_max, 
		      float x1, int *group_member, int *indx, int ngal, float radius, float mass, 
		      int igrp, float *luminosity, float *nsat_cur, int i1, float *prob_total)
{
  int j, j1, id;
		      ...
		      int igrp, float *luminosity, float *nsat_cur, int i1, float *prob_total)
{
  ...
  *nsat_cur = 0;
  ...
  // do not do a volume correction for the cubic mocks!!!!  
  dz = fabs(redshift[i]-CZMIN);
  vol_corr = 1-0.5*erfc(dz/(ROOT2*x1));
  *nsat_cur/= vol_corr;
  grp_lum/=vol_corr;
