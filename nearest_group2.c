...
int main(int argc, char **argv)
{
  ...
  nmesh=0; 
  meshlink2D(ngal,&nmesh,0.0,rcube,r,xg,yg,zg,&meshparts,&meshstart,meshfac);
  ...
  for(i=1;i<=ngal;++i)
    {
      ...
      nbrmax=ngal;
      nbrsfind2D(0.0,rcube,r,nmesh,ra[i],dec[i],dec[i],&nbrmax,indx,rsqr,&ragrp[1],&decgrp[1],&zgrp[1],
		    meshparts,meshstart);

      sort2(nbrmax,rsqr,indx);
      ...
      for(i1=0;i1<4;++i1)
	{
	  ...
	  for(j1=0;j1<nbrmax;++j1)
	    {
	      j = indx[j1];
	      if(!(mgrp[j]>MLO && mrg[j]<=MLO+0.5))continue;
	      break;
	    }
	  //print out these results
	  x1 = rcogrp[j]*angular_separation(ra[i],dec[i],ragrp[j],decgrp[j]);
	  fprintf(stdout,"%d %e %f ",j,mgrp[j],x1);
	}
