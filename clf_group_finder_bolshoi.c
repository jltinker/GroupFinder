...
int main(int argc, char **argv)
{
  ...
  float prob_rad, prob_ang, *prob_total, p, x2, x3, x4,err, mass_tolerance = 0.001, prev_mass, xngrp, maxlum, vmax_all;
  ...
  if(argc>3)
    iscatter = atoi(argv[3]);

  fprintf(stderr,"MAGNITUDE: %f\nREDSHIFT: %f\nISCATTER: %d\n",MAGNITUDE,REDSHIFT,iscatter);



  volume = 4./3.*PI*(pow(distance_redshift(REDSHIFT),3.0)-pow(distance_redshift(CZMIN/SPEED_OF_LIGHT),3.0))*0.125;
  fprintf(stderr,"Vol %e %f\n",volume,distance_redshift(REDSHIFT));
  density2halo(0.011);
  ...
  fp = openfile("/Users/tinker/cosmo/CENSAT_DECOMP/BOLSHOI_MOCKS/bolshoi_LC_vacc_z_0.dat");
  ngal = filesize(fp);
  ra = vector(1,ngal);
  dec = vector(1,ngal);
  ...
  for(i=1;i<=ngal;++i) 
    {
      if(iscatter>0)
	fscanf(fp,"%d %f %f %f %f %f %f %f",&idum,&ra[i],&dec[i],&redshift[i],&x1,&x2,&x3,&x4);
      else
	fscanf(fp,"%d %f %f %f %f",&idum,&ra[i],&dec[i],&redshift[i],&x1);
