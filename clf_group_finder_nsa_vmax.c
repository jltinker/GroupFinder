...
int main(int argc, char **argv)
{
  ...
  float prob_rad, prob_ang, *prob_total, p, x2, x3, err, mass_tolerance = 0.001, 
    prev_mass, xngrp, maxlum, cumulative_density;
  ...
  float *H_delta, *Dn4k, *SFR, *sersic, vmax_all;
  ...
  //assume that the area is the full SDSS footprint
  volume = 4./3.*PI*(pow(distance_redshift(REDSHIFT/SPEED_OF_LIGHT),3.0)-pow(distance_redshift(CZMIN/SPEED_OF_LIGHT),3.0))*0.177;
  ...
  for(i=1;i<=ngal;++i) 
    {
      // new read-in statement for NSA catalogs (from Wetzel)
      fscanf(fp,"%d %f %f %f %f %f",&j,&luminosity[i],&ra[i],&dec[i],&redshift[i],&vmax[i]);
      mag_r[i] = -luminosity[i];
      luminosity[i] = pow(10.0,luminosity[i]);
      ra[i] *= PI/180.;
      dec[i] *= PI/180.;
      indx[i] = i;
      redshift[i] *= SPEED_OF_LIGHT;
      vmax[i] *= 1/(9380. / 129600.)*0.177;
      vmax[i] -= 38.0;
      fgets(aa,1000,fp);
    }
  fclose(fp);
  ...
  for(i=1;i<=ngal;++i)
	...
	printf("SKIP %d %f %f\n",i,mag_r[i],redshift[i]/SPEED_OF_LIGHT);
    }
  fflush(stdout);
  ...
  for(i1=1;i1<=ngal;++i1)
    { 
      ...
      printf("NSAT0 %e %e %f %e %f\n",mass[i],group_luminosity[igrp],nsat_indi[i],prob_total[i],sigma[igrp]);
      fflush(stdout);
  ...
  for(i1=1;i1<=ngrp;++i1)
    {
      ...
      x1 = mass[i];
      cumulative_density += 1/vmax[i];
      mass[i] = density2host_halo(cumulative_density);
      rad[i] = pow(3*mass[i]/(4.*PI*DELTA_HALO*RHO_CRIT*OMEGA_M),1.0/3.0);
      ang_rad[i] = rad[i]/distance_redshift(redshift[i]/SPEED_OF_LIGHT);
  ...
  for(niter=1;niter<=niter_max;++niter)
      ...
      for(i1=1;i1<=ngrp;++i1)
	{
	  ...
	  printf("GROUP%d %d %d %e %e %f %e %e %e %e %e %e\n",niter,igrp,i,x1,mass[i],nsat_indi[i],
		 group_luminosity[i1],luminosity[i],prob_total[i],ra[i],dec[i],redshift[i]);
	  fflush(stdout);
...
{
  int j, j1;
  float dx, dy, dz, theta, prob_ang, vol_corr, prob_rad, grp_lum, p0, p0_central,cfac;
  ...
  for(j1=1;j1<=ngal;++j1)
    {
      ...
      prob_rad = exp(-dz*dz/(2*x1*x1))*3.0E+5/(RT2PI*x1);
  ...
  *nsat_cur/= vol_corr;
  ...
  // USING THE CORRECTION FACTOR FOR NSA INCOMPLETENESS:
  if(redshift[i]/SPEED_OF_LIGHT<0.01)return grp_lum;
  cfac = 1.0;
  if(-mag_r[i]>8.5) cfac = 1.97;
  if(-mag_r[i]>9.0) cfac = 1.64;
  if(-mag_r[i]>9.5) cfac = 0.86;
  if(-mag_r[i]>10.0) cfac = 0.77;
  if(-mag_r[i]>10.5) cfac = 0.83;
  if(-mag_r[i]>11.0) cfac = 1.21;
  if(-mag_r[i]>11.5) cfac = 1.71;
  grp_lum *= cfac*(redshift[i]/SPEED_OF_LIGHT-0.01);
