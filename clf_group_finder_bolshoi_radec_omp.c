...
int main(int argc, char **argv)
{
  ...
  // assume that volume is an octant
  volume = 4./3.*PI*(pow(distance_redshift(REDSHIFT),3.0))/8.0;
  volume = volume -  4./3.*PI*(pow(distance_redshift(CZMIN/SPEED_OF_LIGHT),3.0))/8.0;
  ...
  //  fp = openfile("/Users/tinker/cosmo/CENSAT_DECOMP/BOLSHOI_MOCKS/catalog_tr_sc_.16.dat");
  //fp = openfile("/Users/tinker/cosmo/CENSAT_DECOMP/BOLSHOI_MOCKS/catalog.dat");
  fp = openfile("catalog.dat");
  ngal = filesize(fp);
  ra = vector(1,ngal);
  dec = vector(1,ngal);
  ...
  for(i=1;i<=ngal;++i) 
    {
      //fscanf(fp,"%f %f %f %f",&ra[i],&dec[i],&redshift[i],&x1);	
      //mag_r[i] = x1;
      //luminosity[i] = pow(10.0,-0.4*(mag_r[i]+20.44));

      // new read-in statement for  bolshoi_sm_vpeak_sc_.20_sat0.01_z_0_0.083_truth.dat 
      // NB THESE ARE FOR RACHEL'S MOCKS
      //fscanf(fp,"%d %d %f %f %f %e %f",&j,&j,&ra[i],&dec[i],&redshift[i],&x1,&luminosity[i]);
      //ra[i] *= PI/180.;
      //dec[i] *= PI/180.;
      //mag_r[i] = -luminosity[i];
      //luminosity[i] = pow(10.0,luminosity[i]);

      // NB THESE ARE FOR DUNCAN'S CONFORMITY MOCKS
      //fscanf(fp,"%d %f %f %f %f %f %f %f",&j,&ra[i],&dec[i],&redshift[i],&x1,&x1,&x1,&luminosity[i]);
      fscanf(fp,"%d %f %f %f %f %f",&j,&ra[i],&dec[i],&redshift[i],&luminosity[i],&s2n[i]);
      ra[i] *= PI/180.;
      dec[i] *= PI/180.;

      //mag_r[i] = luminosity[i];
      //luminosity[i] = pow(10.0,-0.4*(mag_r[i]+20.44)); // duncan's mocks
      mag_r[i] = -luminosity[i];
      luminosity[i] = pow(10.0,luminosity[i]);
      ...
      switch(iscatter) {
      case -1: ;
      case 1: mag_r[i] = x1; break;
      case 2: mag_r[i] = x2; break;
      case 3: mag_r[i] = x3; break;
      case 4: mag_r[i] = x4; break;
      }
      */
    }
  fclose(fp);
  ...
  for(niter=1;niter<=niter_max;++niter)
      ...
      for(i=1;i<=ngal;++i)
	{
	  ...
	  printf("PROB%d %d %d %d %e %e %e %e %e %e %e %e %e %e\n",niter,i,igrp,group_center[igrp],mag_r[j1],
		 prob_total[i],mass[i],nsat_indi[i],mass[j],nsat_indi[j],theta/ang_rad[j],theta,ang_rad[j],s2n[i]);
	}
